...
static const char Library_create[] =
	...
	"CREATE TABLE track_artist (pid INTEGER NOT NULL, name TEXT, name_order INTEGER, sort_name TEXT, has_songs INTEGER DEFAULT 0, has_music_videos INTEGER DEFAULT 0, has_non_compilation_tracks INTEGER DEFAULT 0, is_unknown INTEGER DEFAULT 0, album_count INTEGER DEFAULT 0, PRIMARY KEY (pid));" \
	...
	"CREATE INDEX item_to_container_container_pid_idx ON item_to_container (container_pid);" \
	"CREATE INDEX item_title_order_idx ON item (title_order);" \
